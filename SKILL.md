...
7. **同步到 Team Skill Platform（自动执行）**
   ...
   **前提条件**（必须全部满足）：
   - 项目存在 `任务清单.md`
   - 配置文件 `<skill-dir>/.sync_config.json` 存在且有效
   - 配置中的 email 和 password 非占位符（不是 `<请填写 xxx>` 格式）
   
   **执行步骤**：
   ```bash
   # 1. 确认 skill 目录
   SKILL_DIR="/root/.gemini/antigravity/skills/spec-task"
   
   # 2. 执行同步
   cd "$SKILL_DIR" && python sync.py -f <项目路径>/任务清单.md
   
   # 或者使用绝对路径
   python /root/.gemini/antigravity/skills/spec-task/sync.py -f <项目路径>/任务清单.md
   ```
   
   **响应提示**：
   - 同步成功：「✅ 任务已完成！效率得分：75.5 🟡 良好，已同步到 Team Skill Platform」
   - 配置无效：「ℹ️ 未配置 Team Skill Platform 凭据，已跳过同步（可在 ~/.gemini/antigravity/skills/spec-task/.sync_config.json 中配置）」
   - 同步失败：「⚠️ 同步失败：[错误信息]，但不影响任务完成」
   
   **注意**：
   - `<skill-dir>` 是当前 SKILL.md 所在目录：`/root/.gemini/antigravity/skills/spec-task`
   - 同步失败不影响任务归档流程
   - 配置优先级：项目级 `.sync_config.json` > 全局配置
